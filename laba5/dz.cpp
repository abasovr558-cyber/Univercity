...
int main() {
    ...
    map<string, Coord> nodes;
    vector<EdgeItem> edges;
    std::tie(nodes, edges) = read_graphml("Wien_road_network.graphml");
    // 2. Задаём названия улиц для начала и конца маршрута
    Coord start, end;
    string name;
    std::tie(start, end, name) = edges[edges.size() - 1];
    string start_street_query = "Suttingergasse";      // Название улицы для старта (пример)
    string end_street_query = "Suttingergasse";      // Название улицы для финиша (пример)
